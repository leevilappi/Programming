...
abstract class Expr {
  ...
  final def expand: Expr = {
    ...
    def toList (t:Expr): List[Expr] =
      t match {
          case Var(n) => List(Var(n))
          case Num(v) => List(v)
          case Multiply(l, r) => toList(l).flatMap( y => toList(r).map(x => (x * y) ))
          case Add(l, r) => toList(l) ++ toList(r)
          case Subtract(l, r) => toList(l) ++ toList( negate(r))
          case Negate(i) => toList(i).map { x => negate(x) }
      }
    
    toList(this).reduceLeft(_ + _).simplify   
  }
}
