 ...
 *
 * http://www.scala-lang.org/api/2.11.8/index.html#scala.collection.Seq
 *
...
package object sequences {
  ...
  def removeOdd(in: Seq[Int]): Seq[Int] = in.filter { x => !(x%2==1)}
  ...
  def mySum(in: Seq[Int]): Int = in.reduceLeft( (a,b) => sumOfTwo(a,b))
//  def mySum(in: Seq[Int]): Int = in.reduceLeft(_ + _)
  ...
  def sumOfSquaresOfOdd(in: Seq[Int]): Int = 
    in.filter(???)         // (1)  [when making a cascade it is convenient ..]
      .map(???)            // (2)  [.. to break the cascade into multiple]
      .reduce(???)         // (3)  [.. lines to ease readability]
