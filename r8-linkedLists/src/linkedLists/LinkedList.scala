...
abstract class LinkedList[A]() {
   ...
   */
  def zip[B](that: LinkedList[B]): LinkedList[Pair[A, B]] = {
    @tailrec def inner(l1: LinkedList[A], l2: LinkedList[B], result: LinkedList[Pair[A, B]]): LinkedList[Pair[A, B]] = {
      if (l1.isEmpty || l2.isEmpty) result
      else inner(l1.tail, l2.tail, Cons((l1.head, l2.head), result))
    }
    inner(this, that, Nil[Pair[A, B]]()).reverse
  }
}
