...
object factory {
  ...
  def buildEqual(bus1: Bus, bus2: Bus): Gate = {
    require(bus1.length == bus2.length, 
     ...
     var x = abus1.zip(abus2).map(x => x._1 && x._2)
     
     var y = bus1.zip(bus2).map(x => x._1 && x._2)
     
     x.zip(y).map(x => x._1 || x._2).reduceLeft(_ && _)
  }
   ...
   *        (!b1i && !b2i && giminus1) || (!b1i && b2i && !gminus1) || (!b1i && b2i && giminus1) || (b1i && b2i && giminus1)
   *      }
   *
  ...
  def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    var lastGate = Gate.False
    
    for(i <- 0 until bus1.length){
      lastGate = buildGategi(bus1(i), bus2(i), lastGate )
    }
  ...
  def buildCountTrueBits(bus: Bus): Bus = {
    ...
    var emptyBus = Bus.falses( log2ceil(bus.length))
    
    for(i <- 0 until bus.length){
      emptyBus = buildAdderNoOverflow(emptyBus, new Bus(Array(bus(i))))
    }
    
    emptyBus.take(log2ceil(bus.length))
  ...
  def buildMajorityTester(bus: Bus): Gate = {
    ...
    var abus = ~bus
    var x = buildCountTrueBits(bus)
    var y = buildCountTrueBits(abus)
      
    buildUnsignedLess(y, x)
  }
  ...
  def buildMultiplier(bus1: Bus, bus2: Bus): Bus = {
    ...
    var l = Bus.falses(bus1.length + bus2.length)
    ...
    for(i <- 1 until bus1.length){
      row = buildAdder(row, Bus.falses(i) ++ multiply(bus1(i), bus2) ++ Bus.falses(bus1.length - i) )
    }
   row 
  }
