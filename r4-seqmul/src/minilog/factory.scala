...
object factory {
  ...
  def buildSequentialMultiplier(aa: Bus, bb: Bus, loadEnable: Gate): (Gate, Bus) = {
    ...
    //    mA << 1
    //    mB >> 1
    mA.buildFeedback( buildTwoBusSelector(loadEnable.host.falses(1) ++ mA.dropRight(1), aa ++ loadEnable.host.falses(bb.length), loadEnable) )
    mB.buildFeedback( buildTwoBusSelector(mB.drop(1) ++ loadEnable.host.falses(1),bb ++ loadEnable.host.falses(aa.length), loadEnable))
    
    res.buildFeedback( buildTwoBusSelector(buildAdder(res, mA && mB(0)), loadEnable.host.falses(aa.length + bb.length), loadEnable) )       
