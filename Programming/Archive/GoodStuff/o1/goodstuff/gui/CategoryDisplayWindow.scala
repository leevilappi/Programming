...
/** Class `CategoryDisplayWindow` represents windows that display a single category 
  ...
  */
class CategoryDisplayWindow(val category: Category) extends Frame {
  window =>
  ...
  private def requestExperienceInfo() = {
    val name        = requestNonEmptyLine("Enter " + this.category.name.toLowerCase + " name:", "A name is required.", this.contents.headOption).getOrElse("N/A")
    val price       = requestDouble("Enter the price per " + this.category.unit + " in euros:", _ >= 0, "A non-negative number is required.", this.contents.headOption).getOrElse(0.0)
    val description = requestAnyLine("Enter your description of " + name + ":", this.contents.headOption).getOrElse("")
    val rating      = requestInt("Rate the " + this.category.name.toLowerCase + ":", _ >= 0, "A non-negative integer is required.", this.contents.headOption).getOrElse(0)
