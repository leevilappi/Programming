...
object aliohjelmia {
  ...
  private lazy val movieData = {
    ...
    val sourceUrl = Option(this.getClass.getResource(s"/$Subdir/$FileName")).getOrElse(throw new Exception(s"Could not read the file $FileName, which is expected to be under $Subdir.")) 
    val lines     = io.Source.fromURL(sourceUrl, "UTF-8").getLines.toList.map( _.split(";") )
  ...
  def pelaaPylpyrapelia(pelaaja: String) = {
    ...
    def pyydaValinta(jaljella: Int) = {
      requestInt("Jäljellä on " + jaljella + " pylpyrää. Montako otat?", luku => (luku == 1 || luku == 2), "Ota 1 tai 2.", None)
    }
    
    display("Tervetuloa PYLPYRÄÄTTÖRIIN, " + pelaaja + "!\nAlussa on kaksitoista pylpyrää.\n" +
            "Pelaajat ottavat vuorotellen 1 tai 2 pylpyrää.\nViimeisen pylpyrän saanut voittaa.", None) 
    var jaljella = 12
    while (jaljella > 0) {
      pyydaValinta(jaljella) match {
        case Some(valittu) =>
          ...
          display("Otan " + koneOtti + " " + (if (koneOtti == 1) "pylpyrän" else "pylpyrää") + ".", None)
          jaljella -= koneOtti
        case None =>
          jaljella = 0
      } 
