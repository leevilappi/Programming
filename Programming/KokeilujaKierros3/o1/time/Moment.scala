...
/** Each instance of the class `Moment` represents a "moment" on a time scale,
  ...
  *
  * @param time  an integer indicating which moment the object should represent (2016, for instance) */
class Moment(private val time: Int) {
  ...
  def isLaterThan(interval: Interval): Boolean = if (this.isLaterThan(interval.end)) true else false

  def isIn(interval: Interval): Boolean = if (interval.contains(this)) true else false
}
