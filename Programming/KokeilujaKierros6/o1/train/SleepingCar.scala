...
class SleepingCar(val numberOfCabins: Int) extends TrainCar {
  ...
  private val reservedCabinsCount = 0
  ...
  def numberOfFreePlaces = {
   // turha rivi, estää kaikkien paikkojen läpikäynnin val freeCount = this.emptyCabinCount                         // temporary
   var free = 0                                                 // gatherer
   for (cabinNumber <- 1 to numberOfCabins) {                        // stepper
  ...
  def reserveCabin(cabinNumber: Int): Boolean = {
    if (!this.isEmptyCabin(cabinNumber)) {
      ...
      this.cabinReservations(cabinNumber - 1) = 3

      true
      // turha rivi if (this.reserveCabin(cabinNumber)) true else false
    }
  }
