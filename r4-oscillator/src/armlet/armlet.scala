...
object armlet {
  ...
  def buildProcessor(reset_e: Gate,
    ...
    val (instr_exec, instr_wait, immed_in, wait_e) = buildILU(read_in,last_instr,oc_e,lsw_e,reset_e)

    // Build instruction decode unit

    val (l_idx, a_idx, b_idx, i_in, ia_e, ib_e, to_ALU_builder, loa_e, sto_e, cmp_e, to_JBU_builder, hlt_e, trp_e) = buildIDU(instr_exec, immed_in)
    ...
    val (reg_postALU, f_eq, f_ab, f_gt, opA) = buildALU(reg_postLCU, l_idx, a_idx, b_idx, i_in, ia_e, ib_e, to_ALU_builder, reset_e)

    // Build jump and branch unit

    val (pc_out, psr_out) = buildJBU(pc_in, psr_in, opA, cmp_e, to_JBU_builder, f_eq, f_ab, f_gt, hlt_e || lsw_e, reset_e)

    // Build memory interface unit

    val (reg_out, out_l_idx, out_lcu_e, out_lsw_e, mem_read_e, mem_write_e, mem_addr, mem_data) = buildMIU(reg_postALU, l_idx, a_idx, loa_e, sto_e, pc_out, !(loa_e || sto_e), reset_e)
  ...
  def buildDataPath(read_in: Bus,
    ...
    val (l_idx, a_idx, b_idx, i_in, ia_e, ib_e, to_ALU_builder, loa_e, sto_e, cmp_e, to_JBU_builder, hlt_e, trp_e) = buildIDU(instr_in, immed_in)
    ...
    val (reg_postALU, f_eq, f_ab, f_gt, opA) = buildALU(reg_postLCU, l_idx, a_idx, b_idx, i_in, ia_e, ib_e, to_ALU_builder, host.False)

    // Build memory interface unit

    val (reg_out, out_l_idx, out_lcu_e, out_lsw_e, mem_read_e, mem_write_e, mem_addr, mem_data) = buildMIU(reg_postALU, l_idx, a_idx, loa_e, sto_e, host.falses(wordlength), host.False, host.False)
  ...
  def buildPlainALU(host: Circuit) = {
    ...
    val to_trigger = Vector(("L", l_idx.reverse, Trigger.intDecoder(l_idx)),("A", a_idx.reverse, Trigger.intDecoder(a_idx)),("B", b_idx.reverse, Trigger.intDecoder(b_idx)),("operation", new Bus(ALU_es), opDecoder(ops_ALU.map(_._1),new Bus(ALU_es))),("imm_e",Bus(imm_a_e,imm_b_e),immDecoder(imm_a_e,imm_b_e)))
                  
    // Build arithmetic logic unit

    val (reg_out, f_eq, f_ab, f_gt, opA) = buildALU(reg_in, l_idx, a_idx, b_idx, immed_in, imm_a_e, imm_b_e, to_ALU_builder, host.False)
...
class armletPackage() {
  ...
  val (hlt_e, trp_e, mem_read_e, mem_write_e, mem_addr, mem_data, statusString) = armlet.buildProcessor(reset_e, read_in)
...
import javax.swing.{JPopupMenu,AbstractAction,Action}
import java.awt.{Cursor,Toolkit}
import java.awt.datatransfer.{UnsupportedFlavorException,StringSelection,Clipboard,ClipboardOwner,Transferable,DataFlavor}
import java.awt.event.{MouseEvent,MouseAdapter,ActionEvent,InputEvent}


class Ticker(val startText: String = "") extends SwingApplication {
  ...
  val prgtext = new TextArea {
    ...
    text = if (startText == "") defaultStartText else startText
    columns = 58
    rows = 15
    font = fontC
  }
  ...
  prgtext.reactions += {
     case e: MouseClicked if(e.peer.getButton == 
        ...
        if(b == assemblebutt) {
          ...
          asmresult match {
            case (Some(code),str) =>
              ...
              bintext.text = ";;\n;; binary image -- ready to load\n;;\n;; %s\n;;\n\n%s".format(new Date, armlet.disassemble(code))
