 ...
 *
 * `StereoSound` can be read eather from file or created from two audio data channels.
 *
...
class StereoSound(val fileName: String, val left: ArrayBuffer[Int], val right: ArrayBuffer[Int]) {
  ...
  def this(fileName: String) {
    this(fileName, StereoSound.getLeftChannel(fileName), StereoSound.getRightChannel(fileName))
  }
   ...
   */
  def play() : Unit = {
    val data: Array[Byte] = StereoSound.mixChannels(left, right)
...
/**
 * Companion object for StereoSound. Contains methods for separating and mixing back together left and
