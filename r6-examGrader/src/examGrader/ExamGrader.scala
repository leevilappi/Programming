...
class ExamGrader(val studentDB: StudentDB,
  ...
  def points(id: StudentId): Int = {
    ...
    examPoints.filter(x => x._1 == id).values.flatten.sum + bonusPoints.filter(x => x._1 == id).values.sum
   ...
   */
  lazy val grades: Map[StudentId, Int] = graded.zip(graded.zip(graded).toMap.values.map(x => pointsToGrade( points(x) ) ) ).toMap
  ...
  def bestStudents(n: Int): Seq[StudentId] ={
    grades.keys.map(x => (x , points(x))).toVector.sortBy(x => x._2).reverse.take(n).map(x => x._1).toSeq
  }
  ...
  def studentsByGrade: Map[Int, Set[StudentId]] = {
    val test = 0 to 5
    val gradedPupils = grades.groupBy( x => x._2).map(x => (x._1, x._2.map(x => x._1).toSet))
    test.map { x => (x, gradedPupils.getOrElse(x, Set()))}.toMap
  }
  ...
  def gradesByProgramme: Map[StudyProgramme, Map[StudentId, Int]] =  {
    grades.groupBy(x => studentDB.programmeOf(x._1))
  }
