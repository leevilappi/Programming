...
class RobotBody(val world: RobotWorld, initialLocation: Coords, initialFacing: Direction) { 
  ...
  def canMoveTowards(direction: Direction) = !this.neighboringSquare(direction).isUnpassable
  ...
  /** Determines whether the robot is stuck or not, that is, if there are any squares around 
    ...
    * for this purpose.
    * @see [[canMoveTowards]] */
  def isStuck = !Direction.Clockwise.exists( this.canMoveTowards(_) )
  // alternative implementation: this.world.neighbors(this.location, false).forall( _.isUnpassable )
